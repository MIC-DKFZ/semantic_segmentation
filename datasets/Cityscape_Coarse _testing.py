...
import numpy as np
import random
from datasets.Cityscape import Cityscape_dataset,show_cityscape

class Cityscape_coarse_dataset(Cityscape_dataset):
    def __init__(self,root,split="train",transforms=None,coarse_portion=1.0):
        ...
        log.info("Dataset: Cityscape train (Coarse+Fine) | Total: %s images - %s masks | Fine: %s images - %s masks | Fine: %s images - %s masks", len(self.imgs),len(self.masks), len(imgs_fine),len(masks_fine),len(imgs_coarse),len(masks_coarse))
