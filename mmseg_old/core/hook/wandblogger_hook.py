...
class MMSegWandbHook(WandbLoggerHook):
        ...
        if self.log_evaluation or self.log_checkpoint_metadata:
            if self.eval_hook is None:
                ...
                runner.logger.warning(
                    ...
                    "MMSegWandbHook, `EvalHook` or `DistEvalHook` in mmseg_old "
    ...
    def _add_ground_truth(self, runner):
        ...
        for idx in self.eval_image_indexs:
            ...
            if seg_mask.ndim == 2:
                wandb_masks = {
                    "ground_truth": {"mask_data": seg_mask, "class_labels": self.class_id_to_label}
                }
                ...
                runner.logger.warning(
                    f"The segmentation mask is {seg_mask.ndim}D which is not supported by W&B."
                )
                self.log_evaluation = False
                return
    ...
    def _log_predictions(self, results, runner):
        ...
        for ndx, eval_image_index in enumerate(self.eval_image_indexs):
            ...
            if pred_mask.ndim == 2:
                wandb_masks = {
                    "prediction": {"mask_data": pred_mask, "class_labels": self.class_id_to_label}
                }
                ...
                self.eval_table.add_data(
                    ...
                    self.wandb.Image(
                        self.data_table_ref.data[ndx][1], masks=wandb_masks, classes=self.class_set
                    ),
                )
            else:
                runner.logger.warning(
