...
class ConcatDataset(_ConcatDataset):
    ...
    def __init__(self, datasets, separate_eval=True):
        ...
        assert separate_eval in [
            True,
            False,
        ], f"separate_eval can only be True or False,but get {separate_eval}"
        if any([isinstance(ds, CityscapesDataset) for ds in datasets]):
            raise NotImplementedError(
                "Evaluating ConcatDataset containing CityscapesDatasetis not supported!"
            )
    ...
    def evaluate(self, results, logger=None, **kwargs):
        ...
        if self.separate_eval:
            ...
            for size, dataset in zip(self.cumulative_sizes, self.datasets):
                ...
                print_log(
                    f"\nEvaluateing {dataset.img_dir} with {len(results_per_dataset)} images now",
                    logger=logger,
                )
    ...
    def format_results(self, results, imgfile_prefix, indices=None, **kwargs):
        ...
        for i, indice in enumerate(indices):
            ...
            res = self.datasets[dataset_idx].format_results(
                [results[i]], imgfile_prefix + f"/{dataset_idx}", indices=[sample_idx], **kwargs
            )
            ret_res.append(res)
        return sum(ret_res, [])
