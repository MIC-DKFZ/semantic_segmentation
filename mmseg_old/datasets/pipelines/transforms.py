...
class Pad(object):
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f"(size={self.size}, size_divisor={self.size_divisor}, pad_val={self.pad_val})"
        return repr_str
...
class CLAHE(object):
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f"(clip_limit={self.clip_limit}, tile_grid_size={self.tile_grid_size})"
        return repr_str
...
class RandomRotate(object):
    ...
    def __init__(self, prob, degree, pad_val=0, seg_pad_val=255, center=None, auto_bound=False):
        ...
        if isinstance(degree, (float, int)):
            ...
            self.degree = degree
        assert len(self.degree) == 2, f"degree {self.degree} should be a tuple of (min, max)"
...
class RGB2Gray(object):
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f"(out_channels={self.out_channels}, weights={self.weights})"
        return repr_str
...
class AdjustGamma(object):
    ...
    def __init__(self, gamma=1.0):
        ...
        self.gamma = gamma
        inv_gamma = 1.0 / gamma
        self.table = np.array([(i / 255.0) ** inv_gamma * 255 for i in np.arange(256)]).astype(
            "uint8"
        )
...
class RandomMosaic(object):
    ...
    def __init__(
        self, prob, img_scale=(640, 640), center_ratio_range=(0.5, 1.5), pad_val=0, seg_pad_val=255
    ):
        assert 0 <= prob and prob <= 1
