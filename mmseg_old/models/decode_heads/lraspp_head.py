...
class LRASPPHead(BaseDecodeHead):
    ...
    def __init__(self, branch_channels=(32, 64), **kwargs):
        ...
        if self.input_transform != "multiple_select":
            raise ValueError(
                ...
                "must be 'multiple_select'. But received "
        ...
        for i in range(len(branch_channels)):
            self.convs.add_module(
                f"conv{i}", nn.Conv2d(self.in_channels[i], branch_channels[i], 1, bias=False)
            )
        ...
        self.image_pool = nn.Sequential(
            ...
            ConvModule(
                self.in_channels[2], self.channels, 1, act_cfg=dict(type="Sigmoid"), bias=False
            ),
        )
    ...
    def forward(self, inputs):
        ...
        for i in range(len(self.branch_channels) - 1, -1, -1):
            x = resize(
                x, size=inputs[i].size()[2:], mode="bilinear", align_corners=self.align_corners
            )
