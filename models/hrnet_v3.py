...
class HighResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.last_layer = nn.Sequential(
            nn.Conv2d(
                in_channels=self.high_level_ch,
                out_channels=self.high_level_ch,
                kernel_size=1,
                stride=1,
                padding=0,
            ),
            Norm2d(self.high_level_ch, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=relu_inplace),
            nn.Conv2d(
                in_channels=self.high_level_ch,
    ...
    def forward(self, x_in):
        ...
        x_size = x_in.size(2), x_in.size(3)
        ...
        feats = self.last_layer(feats)
        feats = F.interpolate(feats, size=x_size, mode="bilinear", align_corners=align_corners)
...
def get_seg_model(cfg):
    ...
    model.init_weights(cfg.MODEL.PRETRAINED_WEIGHTS)
