...
def randAugment_test(N: int, M: int, p: float = 0.5, bins: int = 10, mode: int = 0) -> object:
    ...
    col_transforms = [
        ...
        A.ColorJitter(brightness=0, contrast=0, saturation=color, hue=0, p=p),
        ...
        A.Posterize(num_bits=posterize[M], p=p),
        # Equalize - equivalent to F.equalize(Image.fromarray(img))
        A.Equaliz(mode="pil", p=p),
    ...
    geo_transforms = [
        ...
        A.Affine(rotate=(-rotation, rotation), p=p, mode=mode),
    ]
    ...
    # Augmentations
    color_Aug = [
        A.RandomBrightnessContrast(contrast_limit=(1, 1), brightness_limit=0, p=p),
    ]
