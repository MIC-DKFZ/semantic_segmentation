...
if __name__ == "__main__":
    ...
    Remove the corrupt troisdorf_000000_000073 file
    """
    ...
    # Remove Corrupt file
    corrupt_img = os.path.join(
        root,
        "leftImg8bit_trainextra",
        "leftImg8bit",
        "train_extra",
        "troisdorf",
        "troisdorf_000000_000073_leftImg8bit.png",
    )
    if os.path.exists(corrupt_img):
        os.remove(corrupt_img)

    corrupt_labels = glob.glob(
        os.path.join(
            root,
            "gtCoarse",
            "gtCoarse",
            "train_extra",
            "troisdorf",
            "troisdorf_000000_000073*",
        )
    )
    for file in corrupt_labels:
        os.remove(file)
